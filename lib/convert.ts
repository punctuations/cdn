...
export async function convert(url: string) {
  ...
  return await page.screenshot({
    ...
    clip: {
      ...
      width: Math.min(
        bounding_box ? bounding_box.width : 0,
        page.viewport()?.width ? (page.viewport()?.width as number) : 0
      ),
      height: Math.min(
        bounding_box ? bounding_box.height : 0,
        page.viewport()?.height ? (page.viewport()?.height as number) : 0
      ),
    },
  });
}
